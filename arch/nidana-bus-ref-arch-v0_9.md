...
| **Navigation** | Service | Reads `NavIntent` events from pages, applies guards and deep link resolution, delegates to platform router. See [Section 8.3](#83-navigation-as-a-cross-cutting-concern). |
...
| **NavIntent** | A sealed ADT representing a navigation intent (GoTo, Back, Replace, DeepLink, etc.) published by pages and consumed by the NavigationService. See [Section 8.3](#83-navigation-as-a-cross-cutting-concern). |
...
**nidana-topology** adds the declarative topology layer: the `TopologyBuilder` DSL, `TopologyDefinition` as an inert data structure, the `TopologyGraph` intermediate representation and `toGraph()` introspection, scope declarations, and the activation/deactivation contract. It depends on nidana-core. It is still platform-agnostic and framework-agnostic: it defines the topology abstraction but does not bind it to any platform's lifecycle system. A topology can be activated and deactivated manually using the handle-based API described in [Section 6.3](#63-approach-b-owner-managed-lifecycle) without any runtime adapter.

**nidana-navigation** *(optional)* adds the stream-based navigation model described in [Section 8.3](#83-navigation-as-a-cross-cutting-concern). It contains the `NavIntent` sealed type, `Route` typed references, `NavigationTopics` (intent, currentRoute, history), the navigation guard topology, and the abstract `NavigationExecutor` contract. It depends on nidana-topology (the guard topology uses the builder API). It is platform-agnostic: the concrete executor implementation lives in the platform runtime package. Apps that do not need centralized navigation simply omit this dependency.
...
**nidana-devtools** provides the graph renderers (Mermaid, Graphviz, JSON), topology diff tooling, the envelope inspector, and the build-time topology catalog generator described in [Section 5.5](#55-topology-as-self-documenting-data) and [Section 16.2](#162-devtools-and-observability). It depends on nidana-topology for `TopologyGraph` access.

**nidana-bloc-bridge** *(optional)* provides an adapter for existing Flutter BLoC codebases. It allows a BLoC to subscribe to a topic and republish topic emissions as BLoC state, enabling gradual migration from BLoC to topology-driven UI as described in [Section 12.3](#123-when-interposition-might-be-acceptable). It depends only on nidana-core (for topic subscription) and `flutter_bloc`, not on nidana-topology. A BLoC can subscribe to any topic regardless of which topology writes to it.

**nidana-codegen** *(optional)* generates topic registry classes from a schema file (e.g., `topics.yaml`) as described in [Section 3.5](#35-code-generation-approach-optional). It depends only on nidana-core (for the `Topic<T>` types it generates). It runs as a build-time tool, not a runtime dependency.
...
4. **The reactive library is a hard dependency of core.** nidana-core takes RxDart (for Dart), RxJS (for TypeScript), etc. as a direct dependency. The topology layer and all higher packages inherit it transitively. There is no abstraction layer over reactive primitives. The reactive library is the execution engine, and abstracting it would add complexity without benefit: the topology DSL already hides Rx from consumers ([Section 13.5](#135-platform-decision-matrix-rx-vs-native-reactive)).
