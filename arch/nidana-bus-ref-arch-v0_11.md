...
- [Nidana Bus - A Reactive Data-Flow Architecture for Mobile and Web Applications](#nidana-bus---a-reactive-data-flow-architecture-for-mobile-and-web-applications)
  ...
  - [Appendix C: Library Modularization and Development Phases](#appendix-c-library-modularization-and-development-phases)
    ...
    - [C.6 Cross-Platform Naming Convention](#c6-cross-platform-naming-convention)
...
**Ordering and reentrancy.** Per-topic ordering is guaranteed: a single backing subject delivers values in publication order to all subscribers. Cross-topic ordering is *not* guaranteed: if topology A publishes to topic X and topic Y, subscribers to X and Y may observe the values in either order depending on the reactive engine's scheduling.

**Reentrant publish normalization.** If a subscriber publishes to another topic during handling (a reentrant publish), the bus must defer the inner publish to the next microtask or equivalent scheduling boundary. This normalization is mandatory across all platforms. Without it, the same topology produces different observable behavior on different reactive engines (some deliver synchronously, others defer), silently breaking the portability contract.

The implementation strategy is platform-specific:

- **Dart/RxDart:** Wrap reentrant publishes in `scheduleMicrotask()`.
- **Kotlin/Flow:** Already defers via `StateFlow` semantics. For `SharedFlow`-backed topics, use `emit` within a coroutine to ensure suspension.
- **Swift/Combine:** Dispatch reentrant publishes to the next run loop tick via `DispatchQueue.main.async` or `Combine.receive(on:)`.
- **TypeScript/RxJS:** Wrap reentrant publishes in `queueMicrotask()` or `asapScheduler`.
...
For `StateTopic`, duplicate suppression (`distinctUntilChanged`) is not implicit. Topologies that need it should apply it explicitly.
...
A `Topic<T>` object is a pure typed key: a name, a type tag, and (for `StateTopic`) an initial value. It carries no reactive machinery of its own.

The bus owns all backing reactive subjects. Subject creation is deferred to **first runtime access**: the first `activate()` that includes a `read()` or `write()` for the topic, or the first imperative `publish()`, `observe()`, or `getCurrentValue()` call referencing it. At that point, the bus creates the appropriate backing primitive: a `BehaviorSubject` seeded with the declared initial value for `StateTopic`, a `PublishSubject` for `EventTopic`, or a `ReplaySubject(N)` for `ReplayTopic` (see the variant table in [§2.1](#21-topic)).

Topology declaration does not create subjects. `declare()` runs against a recording builder that captures `read()`/`write()` calls as metadata (see [§5.5](#55-topology-as-self-documenting-data)). The bus interprets this metadata on activation.
...
// declare() captures metadata. No subject created yet.
class AuthTopology extends Topology {
  @override
  final String topologyId = 'auth-core';

  @override
  void declare(TopologyBuilder b) {
    final auth = b.read(AuthTopics.state);   // records TopicRef, returns placeholder
    // ...
  }
}

// Subject created here, on first runtime access during activation.
bus.activate(AuthTopology());
```

No topology "owns" a topic. No explicit registration step is required. Any topology can reference any topic at any time; the bus creates the subject on demand at activation.

#### `getCurrentValue()` Contract

`getCurrentValue(topic: StateTopic<T>): T` is always defined and never throws. If no backing subject exists for the topic, the bus creates one, seeds it with the topic's declared `initial` value (or calls the initial factory once), and returns the result. Subsequent calls return the most recent published value.

This means `getCurrentValue()` is a valid "first runtime access" that triggers subject creation. UI frameworks that require synchronous initial reads (e.g., React's `useSyncExternalStore`, SwiftUI's `@StateObject` initializer) can safely call `getCurrentValue()` before any topology has activated. The topic will hold its declared initial value until a topology or service publishes to it.

`getCurrentValue()` is not defined for `EventTopic` or `ReplayTopic`. Calling it on an `EventTopic` is a compile-time error (enforced by the type system: only `StateTopic<T>` exposes `getCurrentValue()`). `ReplayTopic` exposes `getBufferedValues(): List<T>` instead.
...
topology("checkout-flow") {
  ...
  // Event-driven pipeline: read an event topic, transform, write to output
...
This looks declarative but it is an imperative procedure written in topology syntax. The giveaway is that each intermediate topic has exactly one producer (the previous `write()` in the same topology) and one consumer (the next `read()` in the same topology). The topics are not shared infrastructure; they are thread-safe function call plumbing.
...
This enables `toGraph()` at zero cost: no bus activation, no runtime, no side effects. The bus interprets the same IR for live activation, ensuring the graph and the running system are always structurally identical.
...
| **Topology as sequencer** | The architecture, combined with Section 5.4 guidance, makes the correct pattern (state machine reducer) more natural than the misuse pattern. | A developer implements a multi-step sequential process as a chain of single-consumer read/write steps within a single topology. This is functionally correct but semantically wrong: it pollutes the topic registry with internal implementation details and makes the process harder to test and reason about. |
...
**Static completeness.** All read and write dependencies are captured statically in the `TopologyDefinition` IR. There are no dynamic publish paths invisible to static analysis. Cycle detection, orphan analysis, and scope-violation checks operate on the complete graph. Runtime reentrancy detection (see [§2.3](#23-bus-runtime)) serves as a defense-in-depth layer, not a primary detection mechanism.
...
1. **Extract the topology graph** at build time. Instantiate all topology classes, call `buildDefinition()` to obtain their `TopologyDefinition` IR, then call `toGraph()`. Enumerate all input topics, output topics, and transformer references. No runtime reflection or bus activation needed; the IR is statically available from the recording builder.
...
class CheckoutTopology extends Topology {
  @override
  final String topologyId = 'checkout-flow';
...
### C.6 Cross-Platform Naming Convention

Each platform implementation lives in its own repository. The repository name and package names encode the platform identity to eliminate ambiguity across documentation, issue trackers, and conversation.

| Platform | Repository | Package Prefix | Package Manager | Example Package |
|---|---|---|---|---|
| **Dart/Flutter** | `nidana-dart` | `nidana_dart_` | pub.dev | `nidana_dart_core` |
| **Kotlin/Android** | `nidana-kotlin` | `nidana-kotlin-` | Maven Central | `nidana-kotlin-core` |
| **Swift/iOS** | `nidana-swift` | `Nidana` | SwiftPM | `NidanaCore` |
| **TypeScript/Web** | `nidana-ts` | `@nidana/` | npm | `@nidana/core` |

Each convention follows the target ecosystem's norms: Dart uses underscores, Maven uses hyphens, SwiftPM uses PascalCase, npm uses scoped packages.

The full package name mapping for each platform:

| Architecture Package | Dart (pub.dev) | Kotlin (Maven) | Swift (SwiftPM) | TypeScript (npm) |
|---|---|---|---|---|
| nidana-core | `nidana_dart_core` | `nidana-kotlin-core` | `NidanaCore` | `@nidana/core` |
| nidana-topology | `nidana_dart_topology` | `nidana-kotlin-topology` | `NidanaTopology` | `@nidana/topology` |
| nidana-navigation | `nidana_dart_navigation` | `nidana-kotlin-navigation` | `NidanaNavigation` | `@nidana/navigation` |
| nidana-runtime-{platform} | `nidana_dart_flutter` | `nidana-kotlin-compose` | `NidanaSwiftUI` | `@nidana/react` (or `/angular`, `/vue`) |
| nidana-test | `nidana_dart_test` | `nidana-kotlin-test` | `NidanaTest` | `@nidana/test` |
| nidana-devtools | `nidana_dart_devtools` | `nidana-kotlin-devtools` | `NidanaDevtools` | `@nidana/devtools` |
| nidana-bloc-bridge | `nidana_dart_bloc_bridge` | n/a | n/a | n/a |
| nidana-codegen | `nidana_dart_codegen` | `nidana-kotlin-codegen` | `NidanaCodegen` | `@nidana/codegen` |

Platform-specific packages (bloc-bridge is Dart/Flutter only) have no counterpart on other platforms. Conversely, other platforms may introduce their own platform-specific bridge packages following the same naming pattern.

The reference architecture document and any shared specification artifacts (topic schema files, contract definitions) live in a separate `nidana-spec` repository. Platform implementations reference it as the shared specification but do not share code across repositories.